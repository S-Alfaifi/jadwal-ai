...
export function AddCourseForm({ onSubmit, course }: AddCourseFormProps) {
  const { register, control, handleSubmit, formState: { errors }, watch, setValue } = useForm<CourseFormValues>({
    ...
    defaultValues: course ? 
      ...
      { name: "", sections: [{ id: `section_${Date.now()}`, name: "Section 1", lecture: { days: [], startTime: "09:00", endTime: "10:00", classroom: "" }, hasLab: false }] },
  });
  ...
  const renderSectionFields = (sectionIndex: number, type: 'lecture' | 'lab') => (
    <div className="p-4 border rounded-lg space-y-4 bg-muted/30">
        ...
        <div className="space-y-2">
            <Label htmlFor={`sections.${sectionIndex}.${type}.classroom`}>Classroom (Optional)</Label>
            <Input {...register(`sections.${sectionIndex}.${type}.classroom`)} placeholder="e.g. Room 101" />
      ...
      <ScrollArea className="h-[400px] pr-4">
        <div className="space-y-6">
         ...
         <Button
            ...
            onClick={() => append({ id: `section_${Date.now()}`, name: `Section ${fields.length + 1}`, lecture: { days: [], startTime: "09:00", endTime: "10:00", classroom: "" }, hasLab: false})}
          >
